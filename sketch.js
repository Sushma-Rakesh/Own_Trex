...
function setup(){
  ...
  trex=createSprite(150,windowHeight/2,20,0);
  trex.addAnimation("running",trex_running);
  trex.addAnimation("Out",trexOutImg)
  trex.scale=0.5;

 ground=createSprite(200,windowHeight/2,1500,20);
 ...
 ground.x=ground.width/2
  ...
  invisibleGround=createSprite(200,(windowHeight/2)+10,2000,20)
...
function draw(){
  background("white")
  textSize(20)
  text("Score : "+score,windowWidth-130,20)
...
if((touches.length>0 || keyDown("SPACE")) && trex.y>=windowHeight/2-20){
  trex.velocityY=-10;
  jumpSound.play();
  touches = [];
}
...
function spawnClouds(){
  if(frameCount%60===0){
    cloud=createSprite(1500,(windowHeight/2)-60,10,10)
    cloud.y=Math.round(random(windowHeight/2-300,windowHeight/2-100))
    cloud.velocityX=-3
...
function spawnObstacles(){
  if(frameCount%50===0){
    var obstacle=createSprite(1500,(windowHeight/2)-20,10,10)
